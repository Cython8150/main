...
void captureImageAsync() {
    ...
    thread([&]() {
        try {
            ...
            // 获取当前关节角度（在子线程中保存副本）
            vector<double> joint_angles_copy(6);
            for (int i = 0; i < 6; i++) {
                joint_angles_copy[i] = g_joint_targets[i];
            }
            ...
            try {
                ...
                if (image.empty()) {
                    cerr << "获取的图片为空" << endl;
                } else if (duration.count() > 5000) {  // 超过5秒
            ...
            filename << g_save_path << "/img_" << setfill('0') << setw(3) << counter;
            
            // 添加关节角度到文件名
            for (int i = 0; i < 6; i++) {
                double angle = joint_angles_copy[i];
                double deg = angle * 180.0 / 3.14159;
                filename << "_" << fixed << setprecision(4) << deg;
            }
            filename << ".jpg";
            ...
            if (save_success) {
                ...
                // 保存角度信息
                ofstream info_file(g_save_path + "/joint_angles.txt", ios::app);
                if (info_file.is_open()) {
                    info_file << "Image " << counter << ":\n";
                    info_file << "  File: " << filepath << "\n";
                    info_file << "  Joint angles (rad): ";
                    for (int i = 0; i < 6; i++) {
                        info_file << fixed << setprecision(4) << joint_angles_copy[i];
                        if (i < 5) info_file << " ";
                    }
                    info_file << "\n";
                    info_file << "  Joint angles (deg): ";
                    for (int i = 0; i < 6; i++) {
                        info_file << fixed << setprecision(2) << (joint_angles_copy[i] * 180.0 / 3.14159);
                        if (i < 5) info_file << " ";
                    }
                    info_file << "\n\n";
                    info_file.close();
...
int main() {
    ...
    try {
        ...
        // 4. 创建保存目录
        int t = time(nullptr);
        g_save_path = "image/hand_eye_" + to_string(t);
