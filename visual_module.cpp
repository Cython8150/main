...
cv::Mat get_rgb_picture(RemoteAPIObject::sim& sim, int visionSensorHandle) {
    ...
    cv::Mat image = cv::Mat(res[1], res[0], CV_8UC3, img.data()).clone();
...
cv::Mat get_depth_picture(RemoteAPIObject::sim& sim, int visionSensorHandle) {
    ...
    // depth_buffer 在返回后同样可能失效，先拷贝再归一化显示
    cv::Mat depth_copy = cv::Mat(resolution[1], resolution[0], CV_32F, depth_buffer.data()).clone();
    cv::Mat image;
    cv::normalize(depth_copy, image, 0, 255, cv::NORM_MINMAX, CV_8U);
    cv::flip(image, image, 0);
...
cv::Point3f point_cam2base(const cv::Point3f& cam_point) {
    static const char* kHandEyeEyeToHandXml = "./calibration_results/hand_eye_eye_to_hand_result.xml";
    static cv::Mat T_base_from_cam;
    static bool s_loaded = false;

    if (cam_point.x == -1.f && cam_point.y == -1.f && cam_point.z == -1.f)
        return cam_point;

    if (!s_loaded) {
        cv::FileStorage fs(kHandEyeEyeToHandXml, cv::FileStorage::READ);
        if (!fs.isOpened()) {
            std::cerr << "point_cam2base: 无法打开手眼文件 " << kHandEyeEyeToHandXml << std::endl;
            return cv::Point3f(-1.f, -1.f, -1.f);
        }
        fs["T_base_from_cam"] >> T_base_from_cam;
        fs.release();
        if (T_base_from_cam.empty() || T_base_from_cam.rows != 4 || T_base_from_cam.cols != 4) {
            std::cerr << "point_cam2base: 缺少 T_base_from_cam，请先运行 to_hand_calibration" << std::endl;
            return cv::Point3f(-1.f, -1.f, -1.f);
        }
        T_base_from_cam.convertTo(T_base_from_cam, CV_64F);
        s_loaded = true;
    }

    // detectObject3D 为对齐 CoppeliaSim 视觉系对 X、Y 做了取反；solvePnP/手眼标定仍是标准 OpenCV 相机系。
    // T_base_from_cam 将 OpenCV 相机系点变到基座，故此处先变回 OpenCV 系再乘矩阵。
    cv::Mat p_c = (cv::Mat_<double>(4, 1) << -static_cast<double>(cam_point.x), -static_cast<double>(cam_point.y),
                 static_cast<double>(cam_point.z), 1.0);
    cv::Mat p_b = T_base_from_cam * p_c;
    return cv::Point3f(static_cast<float>(p_b.at<double>(0)), static_cast<float>(p_b.at<double>(1)),
                       static_cast<float>(p_b.at<double>(2)));
}
